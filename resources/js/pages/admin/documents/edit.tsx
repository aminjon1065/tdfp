    ...
    return (
        <AdminLayout
            breadcrumbs={[
                ...
                { title: t(locale, 'admin.form.edit'), href: `/admin/documents/${document.id}/edit` },
            ]}
        >
            <Head title={`${t(locale, 'admin.form.edit')} ${t(locale, 'admin.content.documents')}`} />
            <div className="max-w-3xl space-y-6">
                <h1 className="text-2xl font-bold">{t(locale, 'admin.form.edit')} {t(locale, 'admin.content.documents')}</h1>
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-2">
                        <Label>{t(locale, 'admin.form.replaceFileOptional')}</Label>
                        <input
                            type="file"
                        ...
                        {document.file_name && (
                            <p className="text-xs text-muted-foreground">
                                {t(locale, 'admin.form.currentFile')}: {document.file_name}
                            </p>
                        )}
                    </div>
                    ...
                    <div className="flex gap-3">
                        <Button type="submit" disabled={processing}>
                            {processing ? t(locale, 'admin.content.saving') : `${t(locale, 'common.update')} ${t(locale, 'admin.content.documents')}`}
                        </Button>
