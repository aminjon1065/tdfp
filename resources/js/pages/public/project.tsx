...
export default function Project({ projectDocuments }: Props) {
    ...
    const timeline = [
        { date: 'Dec 2024', noteKey: 'project.timeline.step1' as const },
        ...
        >
            ...
            <div className="border-b border-[var(--public-border)] bg-white">
                <div className="gov-container py-5">
                    <div className="flex flex-wrap items-center gap-3 mb-5">
                        ...
                        <span className="text-xs text-slate-400">{t(locale, 'project.worldBankRef')}</span>
                    </div>
                    <dl className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-4">
                        {[
                            { icon: Calendar, labelKey: 'project.approvalDate', value: 'December 17, 2024' },
                            { icon: Calendar, labelKey: 'project.closingDate', value: 'December 13, 2030' },
                            { icon: Landmark, labelKey: 'project.totalFinancing', value: '$39.7M' },
                            { icon: Building2, labelKey: 'project.implementingAgency', value: t(locale, 'site.center') },
            ...
            <div className="sticky top-[57px] z-20 border-b border-[var(--public-border)] bg-white/95 backdrop-blur-sm">
                <div className="gov-container">
                    <nav className="flex gap-0 overflow-x-auto" aria-label="Project sections">
                        {TABS.map((tab) => (
                            <button
                                key={tab.id}
            ...
            <div className="gov-container py-12">
                <div className="grid gap-10 lg:grid-cols-[1fr_320px] lg:items-start">
                    ...
                    <div>
                            ...
                            <section className="space-y-8">
                                <div>
                                    ...
                                    <blockquote className="border-l-4 border-[var(--public-accent)] pl-6">
                                        <p className="text-xl font-medium leading-9 text-[var(--public-primary-hover)]">
                        ...
                        {activeTab === 'components' && (
                            <section>
                                ...
                                <h2 className="gov-section-title mb-6">{t(locale, 'project.financingSources')}</h2>
                                <div className="overflow-hidden rounded-2xl border border-[var(--public-border)]">
                                    <table className="w-full text-sm">
                                        <thead className="bg-[var(--public-surface)] text-xs font-semibold tracking-[0.12em] text-slate-500 uppercase">
                                            <tr>
                                                <th className="px-6 py-3 text-left">{t(locale, 'project.componentLabel')}</th>
                                                <th className="px-6 py-3 text-right">{t(locale, 'project.financing')}</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-[var(--public-border)] bg-white">
                                            ...
                                            <tr className="bg-[var(--public-primary-hover)] font-bold text-white">
                                                <td className="px-6 py-4">{t(locale, 'project.totalFinancing')}</td>
                                                <td className="px-6 py-4 text-right">$51.2M</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                                <p className="mt-3 text-xs text-slate-400">{t(locale, 'project.financingNote')}</p>
                            </section>
                        )}
                        ...
                        {activeTab === 'timeline' && (
                            <section>
                                <p className="gov-kicker mb-6">{t(locale, 'project.timeline')}</p>
                                        ...
                                        <li key={index} className="flex gap-5">
                                            <div className="flex flex-col items-center">
                                                ...
                                                {index < timeline.length - 1 && (
                                                    <div className="mt-1 w-px flex-1 bg-[var(--public-border)]" style={{ minHeight: '2.5rem' }} />
                                                )}
                                            </div>
                                            <div className="pb-8">
                        ...
                        {activeTab === 'results' && (
                            <section>
                                <p className="gov-kicker mb-6">{t(locale, 'project.results')}</p>
                                <div className="grid gap-5 sm:grid-cols-2">
                                    {[
                                        { labelKey: 'project.indicator1', value: '25', targetKey: 'project.indicator1.target' },
                                        { labelKey: 'project.indicator2', value: '28,000', targetKey: 'project.indicator2.target' },
                    ...
                    <aside className="space-y-5">
                        {/* Key facts card */}
                        <div className="rounded-2xl border border-[var(--public-border)] bg-white overflow-hidden">
                            ...
                            <dl className="divide-y divide-[var(--public-border)]">
                                {[
                                    { labelKey: 'project.approvalDate', value: 'Dec 17, 2024' },
                                    { labelKey: 'project.closingDate', value: 'Dec 13, 2030' },
                                    { labelKey: 'project.totalFinancing', value: '$39.7M' },
                                    { labelKey: 'project.idaCredit', value: 'IDA — $30M' },
                                    { labelKey: 'project.sdcGrant', value: 'SDC — $9.7M' },
                                    ...
                                    <div key={labelKey} className="grid grid-cols-[1fr_auto] gap-2 px-5 py-3">
                                        <dt className="text-xs text-slate-400">{t(locale, labelKey)}</dt>
                        ...
                        <div className="rounded-2xl border border-[var(--public-border)] bg-[var(--public-surface)] p-5">
                            ...
                            <div className="space-y-3">
                                <a
                                    href="https://projects.worldbank.org/en/projects-operations/project-detail/P180432"
                                    target="_blank"
