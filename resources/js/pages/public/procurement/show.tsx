        ...
        },
        {
            ...
            itemListElement: [
                ...
                },
                {
                    ...
                    item: currentUrl
                        ? new URL('/procurement', currentUrl).toString()
                        : undefined,
                },
                {
                    '@type': 'ListItem',
    ...
    return (
        ...
        >
            ...
            <article className="container mx-auto max-w-4xl px-4 py-12">
                <Link
                    href={publicHref('/procurement')}
                    className="mb-6 inline-flex items-center text-sm font-medium text-[var(--public-primary)] hover:underline"
                >
                ...
                <div className="mb-8 rounded-xl border bg-white p-6 shadow-sm">
                    <div className="mb-4 flex flex-wrap items-center gap-3">
                        <span className="inline-flex items-center rounded-md bg-[var(--public-primary)] px-3 py-1 text-xs font-semibold tracking-wide text-white uppercase">
                        ...
                        <span className="inline-flex items-center rounded-md border border-[var(--status-warning)]/30 bg-[var(--status-warning)]/8 px-3 py-1 text-xs font-semibold text-[var(--status-warning)]">
                            {getProcurementProcessLabel(
                                procurement.process_state,
                                locale,
                            )}
                        </span>
                        <span className="inline-flex items-center rounded-md border border-[var(--public-primary)]/20 px-3 py-1 font-mono text-xs font-semibold text-[var(--public-primary)]">
                            {procurement.reference_number}
                        </span>
                    </div>

                    <h2 className="mb-4 text-xl font-bold text-[var(--public-text-primary)]">
                        {translation.title ?? t(locale, 'common.untitled')}
                    </h2>
                    <p className="mb-4 text-sm font-medium text-slate-700">
                        {t(locale, procurement.process_summary_key)}
                    </p>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        {procurement.publication_date && (
                            <div className="flex items-center gap-2 text-sm text-gray-600">
                                <Calendar
                                    className="h-4 w-4 shrink-0 text-[var(--public-primary)]"
                                    aria-hidden="true"
                                />
                                <span>
                                    ...
                                    <time
                                        dateTime={procurement.publication_date}
                                    >
                                        {formatLocalizedDate(
                                            procurement.publication_date,
                                            locale,
                        ...
                        {procurement.deadline && (
                            <div className="flex items-center gap-2 text-sm font-medium text-[var(--status-warning)]">
                                ...
                                />
                                <span>
                                    <span>{t(locale, 'common.deadline')}:</span>{' '}
                                    <time dateTime={procurement.deadline}>
                                        {formatLocalizedDate(
                                            procurement.deadline,
                                            locale,
                                        )}
                        ...
                        <div className="text-sm text-gray-600">
                            <span className="font-medium">
                                {procurement.is_submission_open
                                    ? t(
                                          locale,
                                          'procurement.submissionWindowOpen',
                                      )
                                    : t(
                                          locale,
                                          'procurement.submissionWindowClosed',
                                      )}
                            </span>
                            {procurement.days_until_deadline !== null && (
                                <span className="block text-xs text-emerald-700">
                                    {procurement.days_until_deadline}{' '}
                                    {t(locale, 'procurement.daysRemaining')}
                                </span>
                            )}
                ...
                {translation.description && (
                    <div className="mb-8">
                        <h3 className="mb-3 text-lg font-semibold text-[var(--public-primary)]">
                ...
                {procurement.documents && procurement.documents.length > 0 && (
                    <div>
                        <h3 className="mb-4 text-lg font-semibold text-[var(--public-primary)]">
                            {t(locale, 'common.attachedDocuments')}
                        </h3>
                        <div className="space-y-3">
                            {procurement.documents.map((document: any) => {
                                const documentTranslation = getTranslation(
                                    document,
                                    locale,
                                );
                                ...
                                return (
                                    <div
                                        key={document.id}
                                        className="flex items-center justify-between gap-4 rounded-lg border bg-white p-4 transition-shadow hover:shadow-sm"
                                    >
                                            ...
                                            <div>
                                                <p className="text-sm font-medium text-gray-900">
                                                    {documentTranslation.title ??
                                                        `${t(locale, 'common.document')} ${document.id}`}
                                                </p>
